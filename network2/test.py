...
def one_image(name, sv_path, xx, yy, data_original=None, file_name=None, save=False, eval_=True, small=False):
    ...
    ps = 256
    ratio = 2
    file_name = './image/eval' + folder_num + '.txt'
    fil = 0    # filter_name(name)
    ex_time = fits_read(data_original)
    input_image = poisson_noise(data_original, exp_time=ex_time, ratio=ratio, ret=True)
    ...
    if small:
        ...
        output = sess.run(out_image, feed_dict={in_image: in_patch, ex: 5})
    ...
    if not small:
        ...
        for i in range(0, w, step):
            for j in range(0, h,  step):
                ...
                new_image = sess.run(out_image, feed_dict={in_image: in_patch, ex: 5})
