...
for epoch in range(lastepoch, 8001):
    ...
    for name in np.random.permutation(data_list):
        ...
        for r in range(2, 6):
            rat = r
            in_name = glob.glob(gt_dir + name[19: -11] + "*.fits")  # vymysliet nieco lepsie
            in_imag = poisson_noise(in_name[0], exp_time=ex_time, ratio=rat, xx=xx, yy=yy, ps=256,  patch_noise=True, ret=True)
            in_img = np.zeros((ps, ps, 1))
            in_img[:, :, 0] = in_imag
            in_patch = np.expand_dims(in_img, axis=0)
            ...
            _, G_current, output = sess.run([G_opt, G_loss, out_image],
		                                feed_dict={in_image: in_patch, ex: rat, gt_image: gt_patch, lr: learning_rate})
